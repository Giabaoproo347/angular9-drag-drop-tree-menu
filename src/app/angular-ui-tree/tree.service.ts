...
export class TreeService {
  ...
  private static shouldFireLoadNextLevel(tree: Tree): boolean {
    const shouldLoadNextLevel: any =
  ...
  public fireMenuItemSelected(tree: Tree, selectedItem: string | undefined): void {
  ...
  public fireNodeSwitchFoldingType(tree: Tree): void {
    if (tree.isNodeExpanded()) {
      ...
      if (TreeService.shouldFireLoadNextLevel(tree)) {
  ...
  public draggedStream(tree: Tree, element: ElementRef): Observable<NodeDraggableEvent> {
    return this.nodeDraggableService.draggableNodeEvents$.pipe(filter((e: NodeDraggableEvent) => e.target === element))
  ...
  public getController(id: string | number): any {
    if (this.controllers.has(id)) {
      return this.controllers.get(id) as TreeController;
    } else {
      return null;
    }
  }
